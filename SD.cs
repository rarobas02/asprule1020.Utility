using System;
using System.Collections.Generic;
using System.Text;

namespace asprule1020.Utility
{
    public static class SD
    {
        public const string Role_Admin = "IT Admin";
        public const string Role_Client = "Client";
        public const string Role_Evaluator = "Evaluator";
        public const string Role_Po_Head = "PO Head";
        public const string Role_Region_Focal = "Region Focal";

        public const string StatusForReview = "For Review";
        public const string StatusForApproval = "For Approval";
        public const string StatusForReapplication = "For Reapplication";
        public const string StatusForUpdate = "For Update";
        public const string StatusApproved = "Approved";
        public const string StatusReapplication = "Reapplication";
        public const string StatusDuplicate = "Duplicate";

        public const string PendingCount = "SessionPendingCount";
    }
}
